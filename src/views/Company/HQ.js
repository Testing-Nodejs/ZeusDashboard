...
const HQ = () => {
  ...
  const AddHQ = () => {
    if (Country === "-1" || Country === undefined) {
      ...
      Toast.fire({
        icon: "warning",
        title: "Please Enter Head Office!",
      });
    } else {
      document.getElementById("divLoading").className = "show";
      const obj = {
        "HEAD_OFFICE_COUNTRY_FKID": Country,
        "HEAD_OFFICE_STATE_FKID": State,
        "HEAD_OFFICE_DISTRICT_FKID": District,
        "HEAD_OFFICE_TALUK_FKID": City,
        "HEAD_OFFICE_NAME": hq,
      };
      console.log("🚀 ~ file: HQ.js ~ line 268 ~ AddHQ ~ obj", obj)
      axios.post(MyApiUrl + "HOs", obj).then((response) => {
        if (response.data === true) {
          Swal.fire({
            title: "Head Office Added!",
            icon: "success",
          ...
          Swal.fire({
            title: "Head Office Failed To Add!",
            icon: "error",
            confirmButtonText: "OK",
          });
          GetCountry();
  ...
  const EditHq = (id, countryid, stateid, districtid, cityId, hq) => {
  ...
  const UpdateHQ = () => {
    if (Country === "-1" || Country === undefined) {
      ...
      axios.put(MyApiUrl + "HOs/" + pkid + "", obj).then((response) => {
        if (response.data === true) {
          Swal.fire({
            title: "Head Office Details Updated!",
            icon: "success",
            confirmButtonText: "OK",
          });
          GetCountry();
    ...
    if (res) {
      ...
      axios({
        ...
        .then((response) => {
          if (response.data === true) {
            Swal.fire({
              title: "Selected Head Office Deleted!",
              icon: "success",
              confirmButtonText: "OK",
            });
            GetAllHQ();
            ...
            Swal.fire({
              title: "Failed To Delete Head Office!",
              icon: "error",
              confirmButtonText: "OK",
            });
            document.getElementById("divLoading").className = "hide";
  ...
  const GetAllHQ = React.useCallback(() => {
    ...
    axios({
      ...
      .then((response) => {
        console.log("🚀 ~ file: HQ.js ~ line 426 ~ .then ~ response", response)
        const items = response.data.map((item) => {
          return {
            ...item,
            ...
            Head_Office: item.HEAD_OFFICE_NAME,
          };
        });
        setHqData(items);
        document.getElementById("divLoading").className = "hide";
  ...
  return (
    <div id="city">
      ...
      <h1 id="ccmaster">Head Office Master</h1>
          ...
          <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
            <CCardBody>
              <CRow>
                <CCol>
                  <CCard>
                    <CCardHeader>Manage Head Office</CCardHeader>
                    <CCardBody>
                      <CForm
                        action=""
                        method="post"
          ...
          <CCard style={{ boxShadow: "0px 0px 1px 1px #959595" }}>
            <CCardBody>
              <CRow>
                <CCol>
                  <CCard>
                    <CCardHeader>Added Head Office</CCardHeader>
                    <CCardBody>
                      <CDataTable
                        items={HqData}
                        fields={fields2}
                          ...
                          Action: (item) => (
                            <td>
                              <CButton
                                ...
                                onClick={() => {
                                  EditHq(
                                    item.HEAD_OFFICE_PKID,
                                    item.HEAD_OFFICE_COUNTRY_FKID,
                                    item.HEAD_OFFICE_STATE_FKID,
                                    item.HEAD_OFFICE_DISTRICT_FKID,
                                    item.HEAD_OFFICE_TALUK_FKID,
                              ...
                              <CButton
                                size="sm"
                                onClick={() => {
                                  DeleteHQ(item.HEAD_OFFICE_PKID);
                                }}
                                id="war-btn1"
                              >
                                <DeleteSharpIcon />
